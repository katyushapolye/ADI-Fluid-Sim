...
def plotVectorField(field, it, name, x_min=0, x_max=None):
    ...
    X, Y = np.meshgrid(x, y)
    jFinal = int(x_max / dH) if x_max is not None else len(x)
    ...
    # Create the figure
    fig, ax = plt.subplots(figsize=(x_max + 2, 4))
    ...
    ax.set_xlim(x_range)  # Explicitly set the X-axis limits
    ax.set_ylim((0, 1.0))
    ax.set_xlabel("x")  # Increase font size for x-label
    ax.set_ylabel("y")  # Increase font size for y-label
    ax.set_title("Velocity Field - Re = {} - Δh = {:.3f} - Iteration {}".format(RE, dH, int(it)), fontsize=12)  # Increase font size for title

    # Add colorbar (legend) for the magnitude
    cbar = fig.colorbar(img, ax=ax, shrink=0.8, aspect=20, orientation='vertical', pad=0.02)
    cbar.set_label('Magnitude')  # Increase font size for colorbar label

    # Overlay streamlines on the magnitude plot
    seed_x = np.linspace(x[1], x[-2], 10)  # Adjust number of seeds as needed
    seed_y = np.linspace(y[1], y[-2], 10)
    ax.streamplot(
        X[:, :jFinal], Y[:, :jFinal], field[0][:, :jFinal], field[1][:, :jFinal],
        color="black", linewidth=0.5, density=2.0, arrowsize=0.5
    )

    # Set major and minor ticks
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))  # Major ticks with integer values
    ax.xaxis.set_minor_locator(AutoMinorLocator(10))  # Minor ticks: you can adjust the number of subticks

    # Show minor ticks
    ax.tick_params(which='both', axis='x', direction='in', length=6, labelsize=14)  # Increase font size for ticks
...
def main():
    ...
    while stable == False:
        ...
        umid = ADI_U_MID_X( uant, vant, umid)
        vmid = ADI_V_MID_X( uant, vant, vmid)
        ...
        u = ADI_U_FINAL_Y(umid, vmid, u)
        v = ADI_V_FINAL_Y(umid, vmid, v)
