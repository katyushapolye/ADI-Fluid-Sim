...
while time < tF:
    ...
    for i in range(1,N):
        ...
        matrixDiags = tridiagonalX(N-2,bu,i)

        matrix = np.zeros((N-2, N-2))

        np.fill_diagonal(matrix, matrixDiags[1])
        np.fill_diagonal(matrix[1:], matrixDiags[0])
        np.fill_diagonal(matrix[:, 1:], matrixDiags[2])

        umid[i,1:-1] = np.linalg.solve(matrix,font)
    ...
    for i in range(1,N-1):
        ...
        matrixDiags = tridiagonalX(N-1,bv,i)
        ...
        vmid[i,1:-1] = np.linalg.solve(matrix,font)
    ...
    for j in range(1,N-1):
        ...
        matrixDiags = tridiagonalY(N-1,bu,j)
        ...
        u[1:-1,j] = np.linalg.solve(matrix,font)
    ...
    for j in range(1,N):
        ...
        matrixDiags = tridiagonalY(N-2,bv,j)
        ...
        v[1:-1,j] = np.linalg.solve(matrix,font)
...
with open("Error/time_classic.csv", 'a') as file:
